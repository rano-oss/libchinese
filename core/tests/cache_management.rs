...
use libchinese_core::{Config, Engine, Interpolator, Lexicon, Model, NGramModel, UserDict};
...
fn setup_test_engine(cache_size: usize) -> Option<Engine<MockParser>> {
    ...
    let lex =
        Lexicon::load_from_fst_bincode(&data_dir.join("pinyin.fst"), &data_dir.join("pinyin.redb"))
            .ok()?;
