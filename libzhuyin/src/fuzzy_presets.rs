...
pub fn standard_fuzzy_rules() -> Vec<String> {
    ...
    rules.extend(
        [
            ...
            "ㄨㄤ=ㄨㄢ:1.5", // uan/uang
            "ㄨㄣ=ㄨㄥ:1.5",
            "ㄨㄥ=ㄨㄣ:1.5", // un/ong (less common but possible)
        ]
        .iter()
        .map(|s| s.to_string()),
    );
...
mod tests {
    ...
    fn test_hsu_rules_format() {
        ...
        for rule in &rules {
            ...
            assert_eq!(
                parts.len(),
                2,
                "Rule should have format 'X=Y:penalty': {}",
                rule
            );
            ...
            assert_eq!(
                mapping_parts.len(),
                2,
                "Mapping should have format 'X=Y': {}",
                rule
            );

            // Check penalty is a valid float
            let _penalty: f32 = parts[1]
                .parse()
                .expect(&format!("Penalty should be valid float: {}", rule));
        }
    }
    ...
    fn test_hsu_key_confusions() {
        ...
        assert!(
            rules.iter().any(|r| r.contains("ㄓ=ㄐ")),
            "HSU should have ㄓ/ㄐ confusion"
        );
        assert!(
            rules.iter().any(|r| r.contains("ㄔ=ㄑ")),
            "HSU should have ㄔ/ㄑ confusion"
        );
        assert!(
            rules.iter().any(|r| r.contains("ㄕ=ㄒ")),
            "HSU should have ㄕ/ㄒ confusion"
        );
    }
    ...
    fn test_standard_nasal_confusion() {
        ...
        assert!(
            rules.iter().any(|r| r.contains("ㄢ=ㄤ")),
            "Standard should have ㄢ/ㄤ confusion"
        );
        assert!(
            rules.iter().any(|r| r.contains("ㄣ=ㄥ")),
            "Standard should have ㄣ/ㄥ confusion"
        );
    }
    ...
    fn test_bidirectional_rules() {
        ...
        let rules = hsu_fuzzy_rules();
        assert!(
            rules.iter().any(|r| r.starts_with("ㄓ=ㄐ")),
            "Should have ㄓ→ㄐ"
        );
        assert!(
            rules.iter().any(|r| r.starts_with("ㄐ=ㄓ")),
            "Should have ㄐ→ㄓ"
        );
    }
}
