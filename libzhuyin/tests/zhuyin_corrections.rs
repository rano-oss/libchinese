...
fn zhuyin_correct_shuffle_ui_medials() {
    ...
    assert!(
        corrections.contains(&"ㄉㄩㄟ".to_string()),
        "Should suggest ㄩㄟ variant"
    );

    let corrections2 = parser.apply_corrections("ㄉㄩㄟ");
    assert!(
        corrections2.contains(&"ㄉㄨㄟ".to_string()),
        "Should suggest ㄨㄟ variant"
    );
}
...
fn zhuyin_correct_shuffle_un_finals() {
    ...
    assert!(
        corrections.contains(&"ㄓㄩㄣ".to_string()),
        "Should suggest ㄩㄣ variant"
    );

    let corrections2 = parser.apply_corrections("ㄓㄩㄣ");
    assert!(
        corrections2.contains(&"ㄓㄨㄣ".to_string()),
        "Should suggest ㄨㄣ variant"
    );
}
...
fn zhuyin_correct_hsu_zh_j_confusion() {
    ...
    assert!(
        corrections.contains(&"ㄐㄨ".to_string()),
        "HSU: Should suggest ㄐ variant"
    );

    let corrections2 = parser.apply_corrections("ㄐㄩ");
    assert!(
        corrections2.contains(&"ㄓㄩ".to_string()),
        "HSU: Should suggest ㄓ variant"
    );
}
...
fn zhuyin_correct_hsu_ch_q_confusion() {
    ...
    assert!(
        corrections.contains(&"ㄑㄨ".to_string()),
        "HSU: Should suggest ㄑ variant"
    );

    let corrections2 = parser.apply_corrections("ㄑㄩ");
    assert!(
        corrections2.contains(&"ㄔㄩ".to_string()),
        "HSU: Should suggest ㄔ variant"
    );
}
...
fn zhuyin_correct_hsu_sh_x_confusion() {
    ...
    assert!(
        corrections.contains(&"ㄒㄨ".to_string()),
        "HSU: Should suggest ㄒ variant"
    );

    let corrections2 = parser.apply_corrections("ㄒㄩ");
    assert!(
        corrections2.contains(&"ㄕㄩ".to_string()),
        "HSU: Should suggest ㄕ variant"
    );
}
...
fn zhuyin_correct_eten26_zh_z_confusion() {
    ...
    let corrections = parser.apply_corrections("ㄓㄨ");
    assert!(
        corrections.contains(&"ㄗㄨ".to_string()),
        "ETEN26: Should suggest ㄗ variant"
    );

    let corrections2 = parser.apply_corrections("ㄗㄨ");
    assert!(
        corrections2.contains(&"ㄓㄨ".to_string()),
        "ETEN26: Should suggest ㄓ variant"
    );
}
...
fn zhuyin_correct_eten26_ch_c_confusion() {
    ...
    let corrections = parser.apply_corrections("ㄔㄨ");
    assert!(
        corrections.contains(&"ㄘㄨ".to_string()),
        "ETEN26: Should suggest ㄘ variant"
    );

    let corrections2 = parser.apply_corrections("ㄘㄨ");
    assert!(
        corrections2.contains(&"ㄔㄨ".to_string()),
        "ETEN26: Should suggest ㄔ variant"
    );
}
...
fn zhuyin_correct_eten26_sh_s_confusion() {
    ...
    let corrections = parser.apply_corrections("ㄕㄨ");
    assert!(
        corrections.contains(&"ㄙㄨ".to_string()),
        "ETEN26: Should suggest ㄙ variant"
    );

    let corrections2 = parser.apply_corrections("ㄙㄨ");
    assert!(
        corrections2.contains(&"ㄕㄨ".to_string()),
        "ETEN26: Should suggest ㄕ variant"
    );
}
...
fn zhuyin_corrections_in_segmentation() {
    ...
    assert_eq!(
        result.len(),
        2,
        "Should segment into 2 syllables with correction"
    );
...
fn zhuyin_no_corrections_when_exact_match_exists() {
    ...
    assert!(
        result[0].text == "ㄉㄩㄟ" || result[0].text == "ㄉㄨㄟ",
...
fn zhuyin_multiple_corrections_tried() {
    ...
    assert!(
        corrections.contains(&"ㄐㄨ".to_string()),
        "Should have HSU correction"
    );
    assert!(
        corrections.contains(&"ㄗㄨ".to_string()),
        "Should have ETEN26 correction"
    );
...
fn zhuyin_corrections_empty_for_no_patterns() {
    ...
    assert!(
        ...
        "Should have no corrections for ㄅㄚ"
    );
}
