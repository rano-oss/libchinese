...
fn test_data_directory_exists() {
    ...
    assert!(path.exists(), "Data directory {:?} does not exist. CWD: {:?}", path, std::env::current_dir().unwrap());
    
    let lexicon_fst = path.join("lexicon.fst");
    assert!(lexicon_fst.exists(), "lexicon.fst does not exist at {:?}", lexicon_fst);
}

#[test]
fn test_hsu_factory_creates_ime() {
    let result = create_ime_engine_hsu(DATA_DIR, 5);
    assert!(result.is_ok(), "Failed to create HSU IME engine: {:?}", result.err());
}

#[test]
fn test_standard_factory_creates_ime() {
    let result = create_ime_engine_standard(DATA_DIR, 5);
    assert!(result.is_ok(), "Failed to create Standard IME engine: {:?}", result.err());
}

#[test]
fn test_eten_factory_creates_ime() {
    let result = create_ime_engine_eten(DATA_DIR, 5);
    assert!(result.is_ok(), "Failed to create ETEN IME engine: {:?}", result.err());
}

#[test]
fn test_ime_basic_input_flow() {
    let mut ime = create_ime_engine_standard(DATA_DIR, 5)
        .expect("Failed to create IME engine");
    ...
    eprintln!("DEBUG: After ㄋ: preedit='{}', candidates={}", ctx1.preedit_text, ctx1.candidates.len());
    
    eprintln!("DEBUG: Typing ㄧ");
    ime.process_key(KeyEvent::Char('ㄧ'));
    let ctx2 = ime.context();
    eprintln!("DEBUG: After ㄧ: preedit='{}', candidates={}", ctx2.preedit_text, ctx2.candidates.len());
    ...
    eprintln!("DEBUG: Final: preedit='{}', candidates={}", context.preedit_text, context.candidates.len());
    if !context.candidates.is_empty() {
        eprintln!("DEBUG: first candidate='{}'", context.candidates[0]);
    }
    assert!(!context.preedit_text.is_empty(), "Preedit should contain input after 你");
    assert!(context.candidates.len() > 0, "Should have candidates for ㄋㄧˇ (你)");
}
...
fn test_ime_candidate_selection() {
    ...
    let context = ime.context();
    assert!(context.candidates.len() > 0, "Should have candidates for ㄋㄧˇ");
    
    // Select first candidate with space
    ime.process_key(KeyEvent::Space);
    
    let context = ime.context();
    assert!(!context.commit_text.is_empty(), "Should have committed text");
}
...
fn test_ime_number_selection() {
    ...
    if num_candidates >= 2 {
        ...
        let context = ime.context();
        assert!(!context.commit_text.is_empty(), "Should have committed text from number selection");
    }
}
...
fn test_ime_backspace() {
    ...
    let context = ime.context();
    assert!(context.preedit_text.len() < preedit_len_before, "Preedit should be shorter after backspace");
}
...
fn test_ime_escape_clears() {
    ...
    let context = ime.context();
    assert!(context.preedit_text.is_empty(), "Preedit should be cleared after escape");
    assert!(context.candidates.is_empty(), "Candidates should be cleared after escape");
}
...
fn test_ime_page_navigation() {
    ...
    if !context.auxiliary_text.is_empty() && context.auxiliary_text.contains('/') {
        ...
        assert!(!new_candidates.is_empty(), "Should have candidates on next page");
    }
}
...
fn test_ime_cursor_navigation() {
    ...
    if context.candidates.len() > 1 {
        assert!(context.candidate_cursor > initial_cursor, "Cursor should move down");
        
        // Move cursor up
        ime.process_key(KeyEvent::Up);
        
        let context = ime.context();
        assert_eq!(context.candidate_cursor, initial_cursor, "Cursor should return to initial position");
    }
}
...
fn test_hsu_fuzzy_matching() {
    ...
    let mut ime = create_ime_engine_hsu(DATA_DIR, 5)
    ...
    assert!(context.candidates.len() > 0, "HSU should provide candidates with fuzzy matching");
...
fn test_standard_nasal_fuzzy_matching() {
    ...
    assert!(context.candidates.len() > 0, "Standard should provide candidates with nasal fuzzy matching");
}
...
fn test_all_layouts_produce_candidates() {
    ...
    for name in layouts {
        ...
        assert!(result.is_ok(), "{} layout failed to create IME engine", name);
...
fn test_ime_reset_state() {
    ...
    let context = ime.context();
    assert!(context.preedit_text.is_empty(), "Preedit should be cleared after commit");
}
...
fn test_ime_multiple_commits() {
    ...
    assert!(!second_commit.is_empty(), "Second commit should not be empty");
...
fn test_ime_enter_commits() {
    ...
    assert!(context.preedit_text.is_empty(), "Preedit should be cleared after enter");
}

#[test]
fn test_different_page_sizes() {
    for page_size in [3, 5, 10] {
        let mut ime = create_ime_engine_standard(DATA_DIR, page_size)
            .expect("Failed to create IME engine");
