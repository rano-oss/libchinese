...
fn build_fst_and_bincode<P: AsRef<Path>>(
    ...
    let p_kn_bigram = |w1: &str, w2: &str| -> Option<f32> {
        ...
        let p_cont_w2 = if total_bigram_types > 0.0 {
            cont_w2 / total_bigram_types
        } else {
            0.0
        };
        let cint = bigram_counts
            .get(&(w1.to_string(), w2.to_string()))
            .copied()
            .unwrap_or(0);
        let d = match cint {
    ...
    for ((w1, w2, w3), &cnt) in &trigram_counts {
        ...
        let left_cont = uniq_right_for_bigram
            .get(&(w1.clone(), w2.clone()))
            .copied()
            .unwrap_or(0) as f64;
        let cint = cnt;
...
fn estimate_lambdas_for_dataset<P: AsRef<Path>>(
    dataset_dir: P,
    payloads: &[Vec<LexEntry>],
) -> Result<()> {
    ...
    for list in payloads.iter() {
        for entry in list.iter() {
            ...
            for i in 0..chars.len().saturating_sub(2) {
                *trigram_counts
                    .entry((chars[i].clone(), chars[i + 1].clone(), chars[i + 2].clone()))
                    .or_default() += 1;
            }
        }
    }
    ...
    for key in prefixes.iter() {
        let (l1, l2, l3) =
            compute_lambda_for_prefix(&bigram_counts, &trigram_counts, key, &unigram_counts);
...
fn compute_lambda_for_prefix(
    ...
    for ((w1, w2, w3), &c) in trigram_counts.iter() {
        ...
        let p_tri = if c_w1w2 > 1.0 {
            c_w1w2w3_minus / (c_w1w2 - 1.0)
        } else {
            0.0
        };
        let p_bi = if c_w2 > 0.0 {
            c_w2w3 / (c_w2 + total_bi * 0.001)
        } else {
            0.0
        };
        let p_uni = if total_uni > 0.0 {
            c_w3 / total_uni
        } else {
            0.0
        };
    ...
    for ((w1, w2), &c) in bigram_counts.iter() {
        ...
        let p_bi = if c_w1 > 1.0 {
            c_w1w2_minus / (c_w1 - 1.0)
        } else {
            0.0
        };
        let p_uni = if total_uni > 0.0 {
