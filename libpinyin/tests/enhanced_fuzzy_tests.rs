...
fn enhanced_fuzzy_matching_comprehensive_rules() {
    ...
    let temp_path = std::env::temp_dir().join(format!("test_userdict_{}.redb", std::process::id()));
    ...
    assert!(
        candidate_texts.contains(&"字"),
        "Should find exact match for zi->字"
    );
    assert!(
        candidate_texts.contains(&"知"),
        "Should find fuzzy match zi->zhi->知"
    );
    ...
    let candidate_texts: Vec<&str> = candidates.iter().map(|c| c.text.as_str()).collect();
    assert!(
        candidate_texts.contains(&"丝"),
        "Should find exact match for si->丝"
    );
    assert!(
        candidate_texts.contains(&"是"),
        "Should find fuzzy match si->shi->是"
    );
    ...
    let candidate_texts: Vec<&str> = candidates.iter().map(|c| c.text.as_str()).collect();
    assert!(
        candidate_texts.contains(&"反"),
        "Should find exact match for fan->反"
    );
    assert!(
        candidate_texts.contains(&"方"),
        "Should find fuzzy match fan->fang->方"
    );
    ...
    let candidate_texts: Vec<&str> = candidates.iter().map(|c| c.text.as_str()).collect();
    assert!(
        candidate_texts.contains(&"本"),
        "Should find exact match for ben->本"
    );
    assert!(
        candidate_texts.contains(&"崩"),
        "Should find fuzzy match ben->beng->崩"
    );
}
...
fn enhanced_ngram_scoring_with_backoff() {
    ...
    let temp_path =
        std::env::temp_dir().join(format!("test_userdict_ngram_{}.redb", std::process::id()));
    let user = UserDict::new(&temp_path).expect("create test userdict");
    let cfg = libpinyin::PinyinConfig::default().into_base();
    let model = Model::new(lex, ng, user, cfg);
    ...
    assert!(
        candidates_oov.is_empty(),
        "OOV key should return empty results"
    );
}
...
fn enhanced_dp_segmentation_cost_model() {
    ...
    if best.len() == 1 {
        assert_eq!(
            best[0].text, "nihao",
            "Should prefer longer segment 'nihao' over 'ni'+'hao'"
        );
    }
    ...
    assert!(
        all_syllables.contains(&"zi".to_string()),
...
fn fuzzy_penalty_differentiation() {
    ...
    let parser = Parser::with_syllables(&[
        "zh", "z", "ch", "c", "sh", "s", "an", "ang", "en", "eng", "in", "ing",
    ]);
    ...
    for seg_group in [segmentations1, segmentations2] {
        for seg in seg_group {
            assert!(
                !seg.is_empty(),
                "Each segmentation should have at least one syllable"
            );
        }
    }
}
